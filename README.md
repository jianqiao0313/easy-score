...
打开谱面时才加载排版引擎。切换历史乐谱或导入新文件会取消前一份任务在浏览器中的请求和轮询，服务端已受理的识别任务继续处理并保留结果。调整排版失败时会保留原谱面及播放状态，并恢复原来的布局选项。
...
上传先流式写入临时文件，通过校验后移入任务目录。同一服务进程最多同时接收和校验两个上传，额外请求返回 `503` 和 `Retry-After: 1`，稍后可重试；识谱引擎仍串行处理任务。每个任务的读取或保存失败会独立处理，避免阻塞后续队列。
