...
当前稳定版本为 `1.0.4`。x86_64 / amd64 主机复制下面一行即可启动：
...
- 使用钢琴或萨克斯采样音色播放导入后的乐谱，首次使用默认钢琴。
- 支持播放、暂停、跳到上一小节或下一小节、拖动进度、速度、音量和节拍器控制。
- 支持乐谱自动排版，或固定每行显示 1–8 小节。
- 在浏览器中保存音色和每行小节数偏好。
- 对照原 PDF 或图片，并导出 MusicXML 继续校对。

OMR 可能出现错音、漏音或节奏偏差，建议始终对照原谱校验。MusicXML 导入不经过 OMR，目前不承诺覆盖 MusicXML 的全部格式与扩展。1.0.4 的萨克斯使用 32 个独立录音采样，保留自然起音和录音变化，长音使用离线生成的循环点。采样按需加载，超出支持范围或个别采样加载、解码失败时，仅受影响音符回退到合成音色并提示。该选项只改变音色，不会自动移调乐谱。
...
1.0.4 还按需加载谱面排版引擎，降低首屏下载量；切换乐谱时取消旧请求，排版失败时保留原谱面。上传采用流式落盘，同一进程最多同时接收和校验两个上传；额外请求返回 `503`，稍后可重试。单个识谱任务失败不会阻塞后续队列。
...
- Salamander 三角钢琴：Alexander Holm，CC BY 3.0 Unported；萨克斯：Karoryfer 来源、tonejs-instruments 分发，CC BY 3.0 Unported。保留上游原始 MP3、署名、完整许可及处理说明；上游未确认具体演奏者或是否为中音萨克斯。
...
`1.0.4` 镜像在 `/usr/share/doc/easy-score/` 保存应用许可证、声明、Node 许可证及系统包版本清单，在 `/opt/tessdata/LICENSE` 保存 OCR 模型许可证；npm 许可证及音色、字形声明可通过应用的 `/third-party-licenses.txt` 和 `/asset-licenses.txt` 下载。
