...
docker run -d --name easy-score \
  ...
  jianqiao0313/easy-score:latest
```
...
```sh
docker pull jianqiao0313/easy-score:latest
...
1. 上传印刷五线谱 PDF（最大 20 MB），等待识别完成。
2. 选择中音萨克斯或钢琴。首次使用默认中音萨克斯，以后优先读取浏览器保存的选择。
3. 设置每行显示的小节数（1–8），默认 4 小节；小节的音符区域等宽，行首谱号、调号和拍号另留空间，最后一行不会拉伸。设置保存在当前浏览器的 `localStorage` 中。
